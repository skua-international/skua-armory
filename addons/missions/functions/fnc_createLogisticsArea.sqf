...
/*
 * Authors: LinkIsGrim
 * Create a Logistics Area.
 *
 * Arguments:
 * 0: The side that can access this logistics area. If not specified, all sides can access it. <SIDE> (default: sideUnknown)
 ...
 *
 * Example:
 * [sideWest, player, 10, 10, false] call skua_missions_fnc_createLogisticsArea;
 *
 * Public: No
 */

params ["_side", "_center", "_xRadius", "_yRadius", "_isRectangle", "_zRadius"];
TRACE_1("fnc_createLogisticsArea",_this);
...
if (_side == sideUnknown) then {
    ...
    } forEach (values GVAR(logisticsAreas));
} else {
    // Add to specified side
    (GVAR(logisticsAreas) get _side) pushBack _locationArray;
};
