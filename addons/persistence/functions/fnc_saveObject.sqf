...
if (!_forceSave && !isPlayer _object) exitWith {createHashMap};
...
_holder set [HASHKEY_OBJECT_UUID, _object getVariable QGVAR(uuid)];
...
[QGVAR(saveObjectWorld), [_object, _holder]] call CBA_fnc_serverEvent;
...
if (_objectKind == "Unit" && isPlayer _object) then {
    _objectKind = "Player";
};

_holder set [HASHKEY_OBJECT_KIND, _objectKind];
...
[_eventName + "World", [_object, _holder]] call CBA_fnc_serverEvent;
